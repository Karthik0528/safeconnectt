    ...
    try {
      ...
      if (res?.onboarding_required) {
        router.push({
          ...
          params: { email: gUser.email, name: gUser.name, avatar_url: gUser.avatar_url, role: "user" },
        });
      } else {
        router.replace("/(tabs)/home");
      }
  ...
  const handleGoogleSelect = async (gEmail: string, gName: string, gAvatar: string) => {
    ...
    try {
      ...
      if (res?.onboarding_required) {
        router.push({
          pathname: "/auth/google-onboarding",
          params: { email: gEmail, name: gName, avatar_url: gAvatar, role: "user" },
        });
      } else {
        router.replace("/(tabs)/home");
      }
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} contentContainerStyle={styles.content}>
      ...
      <GlassCard style={styles.card}>
        ...
        <GradientButton title="Login as User" onPress={handleLogin} loading={loading} style={{ marginTop: 12 }} />
